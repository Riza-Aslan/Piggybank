...
### Changed
- Use local timezone from Home Assistant configuration (TZ environment variable)
- Set default timezone to Europe/Berlin if TZ not set
- Changed all datetime.utcnow() to datetime.now() to use local time
- Updated model defaults to use datetime.now for created_at timestamps
- This ensures date comparisons and recurring execution work correctly in the user's timezone
...
### Added
- Added GitHub link in the footer of the frontend for easy access to the repository
